...
router.get('/', async (req, res) => {
  try {
    const { search, location, type, category, experience, minSalary, maxSalary,
      remote, featured, page = 1, limit = 10, sort = 'createdAt' } = req.query;
    ...
    const sortObj = sort === 'salary' ? { 'salary.min': -1 } :
      sort === 'views' ? { views: -1 } : { [sort]: -1 };
...
router.get('/:id', async (req, res) => {
  try {
    const job = await Job.findById(req.params.id).populate('employer', 'name company avatar email');
